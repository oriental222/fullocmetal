...
export const RentDate = () => {
  ...
  const hasBlockedDatesInBetweenStartAndEnd = ({ blockedDays, start, end }) => {
    ...
    return Object.keys(blockedDays).some(
  ...
  return (
    <Container>
      <Header>
        ...
        <LineWrapper>
          <TextDate selected={!!lastSelectedDate.timestamp}>
            {rentalPeriod.formatStart}
          </TextDate>
          <SvgArrow width="48" height="10" />
          <TextDate selected={!!lastSelectedDate.timestamp}>
            {rentalPeriod.formatEnd}
          </TextDate>
        </LineWrapper>
      </Header>
      {!isLoading && (
        <Scroll>
          <Calendar
            ...
            markedDates={{ ...selectedDate, ...diasBloqueados }}
          />
        </Scroll>
      )}
      ...
      <BlockButton>
        <Button
          enabled={!!lastSelectedDate.timestamp}
